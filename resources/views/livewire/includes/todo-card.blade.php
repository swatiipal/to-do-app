    ...
    <div wire:key="{{ $todo->id }}"
        ...
        <div class="flex justify-between space-x-2">
            ...
            <div class="flex items-center">
                ...
                @if ($todo->completed)
                    <input class="mr-2" type="checkbox" wire:click="toggle({{ $todo->id }})" checked>
                @else
                    <input class="mr-2" type="checkbox" wire:click="toggle({{ $todo->id }})">
                @endif
                @if ($editingTodoId === $todo->id)
                    <div>
                        <input type="text" placeholder="Todo.." wire:model = "editingTodoName"
        ...
        <div class="mt-3 text-xs text-gray-700">
            @if ($editingTodoId === $todo->id)
                <button class="mt-3 px-4 py-2 bg-teal-500 text-white font-semibold rounded hover:bg-teal-600"
                    wire:click="update">Update</button>
                <button class="mt-3 px-4 py-2 bg-red-500 text-white font-semibold rounded hover:bg-red-600"
